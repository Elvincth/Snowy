...
function serve({ env, options }) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const wwwDir = yield env.project.getSourceDir();
        try {
            ...
            if (options.port !== port) {
                env.log.debug(`Port ${chalk_1.default.bold(String(options.port))} taken, using ${chalk_1.default.bold(String(port))}.`);
                options.port = port;
            }
            if (options.livereloadPort !== livereloadPort) {
                env.log.debug(`Port ${chalk_1.default.bold(String(options.livereloadPort))} taken, using ${chalk_1.default.bold(String(livereloadPort))}.`);
                options.livereloadPort = livereloadPort;
            }
            if (options.notificationPort !== notificationPort) {
                env.log.debug(`Port ${chalk_1.default.bold(String(options.notificationPort))} taken, using ${chalk_1.default.bold(String(notificationPort))}.`);
            ...
            throw new errors_1.FatalException(`${chalk_1.default.green(options.address)} is not available--cannot bind.`);
        }
        const details = [
            `address: ${chalk_1.default.bold(options.address)}`,
            `port: ${chalk_1.default.bold(String(options.port))}`,
            `dev server port: ${chalk_1.default.bold(String(options.notificationPort))}`,
        ];
        if (options.livereload) {
            details.push(`livereload port: ${chalk_1.default.bold(String(options.livereloadPort))}`);
        }
...
function setupServer(env, options) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        let reloadfn;
        if (options.livereload) {
            const { createLiveReloadServer } = yield Promise.resolve().then(() => require('../dev-server'));
        ...
        env.log.debug(`Watch patterns: ${project.watchPatterns.map(v => chalk_1.default.bold(v)).join(', ')}`);
        const watcher = chokidar.watch(project.watchPatterns, { cwd: env.project.directory });
        env.events.emit('watch:init');
        watcher.on('change', (filePath) => {
            env.log.info(`[${new Date().toTimeString().slice(0, 8)}] ${chalk_1.default.bold(filePath)} changed`);
            if (reloadfn) {
                reloadfn([filePath]);
            }
...
function createHttpServer(env, options) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const { DEV_SERVER_PREFIX, injectDevServerScript, injectLiveReloadScript } = yield Promise.resolve().then(() => require('../dev-server'));
        const { LOGGER_STATUS_COLORS } = yield Promise.resolve().then(() => require('../../lib/utils/logger'));
        const [WebSocket, express, proxyMiddleware,] = yield Promise.all([
            Promise.resolve().then(() => require('ws')),
            Promise.resolve().then(() => require('express')),
            Promise.resolve().then(() => require('http-proxy-middleware')),
        ]);
        const app = express();
        const project = yield env.project.load();
        /**
        ...
        if (options.basicAuth) {
            const basicAuth = yield Promise.resolve().then(() => require('basic-auth'));
        ...
        const proxyMiddlewareCfg = { changeOrigin: true, ws: true, logLevel: 'warn', logProvider: () => env.log };
        app.use(pathPrefix, proxyMiddleware(pathPrefix, Object.assign({ target: livereloadUrl, pathRewrite: { [pathPrefix]: '' } }, proxyMiddlewareCfg)));
        if (options.proxy && project.proxies) {
            for (const proxy of project.proxies) {
                const opts = Object.assign({ target: proxy.proxyUrl, pathRewrite: { [proxy.path]: '' } }, proxyMiddlewareCfg);
                if (proxy.proxyNoAgent) {
                    opts.agent = false; // TODO: type issue
                }
                if (proxy.rejectUnauthorized === false) {
                    opts.secure = false;
                }
                app.use(proxy.path, proxyMiddleware(proxy.path, opts));
                env.log.info(`Proxy created ${chalk_1.default.bold(proxy.path)} => ${chalk_1.default.bold(opts.target)}`);
            }
        }
        ...
        wss.on('connection', ws => {
            ws.on('message', (data) => {
                ...
                try {
                    ...
                    env.log.error(`Error parsing JSON message from dev server: "${data}" ${chalk_1.default.red(e.stack ? e.stack : e)}`);
                    return;
                }
                ...
                if (msg.category === 'console') {
                    ...
                    if (status) {
                        ...
                        env.log.msg(`[${chalk_1.default.gray('console.log')}]: ${msg.data.join(' ')}`);
                    }
                    else {
